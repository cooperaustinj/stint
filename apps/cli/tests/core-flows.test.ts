...
import { defaultInvoiceOutputDir, writeTypstInvoice, type InvoicePreviewView } from '../src/lib/invoice'
...
describe('test harness', () => {
    test('initializes in-memory SQLite with all migrations applied', () => {
        ...
        const tables = db
            .query(
                "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('clients','projects','entries','schema_migrations','invoices','invoice_time_items','invoice_expenses')",
            )
...
describe('invoice output defaults', () => {
    test('defaultInvoiceOutputDir uses ~/Downloads', () => {
        expect(defaultInvoiceOutputDir()).toBe(join(homedir(), 'Downloads'))
    })

    test('writeTypstInvoice respects explicit output directory', () => {
        const dir = mkdtempSync(join(tmpdir(), 'stint-invoice-out-'))
        openDirs.push(dir)
        const view: InvoicePreviewView = {
            invoice: {
                id: 1,
                invoiceNumber: '1000',
                status: 'draft',
            ...
            clientBilling: { name: 'Acme Billing' },
            timeItems: [],
            expenses: [],
            totals: { timeSubtotalCents: 0, expenseSubtotalCents: 0, grandTotalCents: 0, totalHours: 0 },
            warnings: [],
            paymentInfoPresent: true,
        }

        const { typPath, pdfPath } = writeTypstInvoice(view, dir)
        expect(typPath.startsWith(dir)).toBe(true)
        expect(pdfPath.startsWith(dir)).toBe(true)
    })
})
...
describe('entry input', () => {
    ...
    test('buildEntryInput: rejects when note is empty', () => {
        expect(() =>
            buildEntryInput({ duration: '1h', note: '  ', client: 'client_key', project: 'project_key' }, {}),
    ...
    test('buildEntryInput: rejects when client is missing and no default exists', () => {
        expect(() => buildEntryInput({ duration: '1h', note: 'work', project: 'project_key' }, {})).toThrow(
            'Client is required',
        )
    })

    test('buildEntryInput: rejects when project is missing and no default exists', () => {
        expect(() => buildEntryInput({ duration: '1h', note: 'work', client: 'client_key' }, {})).toThrow(
            'Project is required',
        )
    })
...
describe('migration and completion', () => {
    ...
    test('completionScript: fish output includes top-level command suggestions', () => {
        ...
        expect(out).toContain(
            'add edit delete restore report list track client project completion migrate config invoice',
        )
